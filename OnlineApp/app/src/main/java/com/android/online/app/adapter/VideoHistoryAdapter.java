...
public class VideoHistoryAdapter extends RecyclerView.Adapter<VideoHistoryAdapter.VideoHistoryViewHolder> {
  ...
  public VideoHistoryAdapter(Context context, List<MovieHistory> movieHistories) {
  ...
  public VideoHistoryViewHolder onCreateViewHolder(ViewGroup viewGroup, int i) {
    View view = LayoutInflater.from(context).inflate(R.layout.video_history_item, viewGroup, false);
  ...
  public void onBindViewHolder(VideoHistoryViewHolder holder, int i) {
    ...
    holder.date.setText(mDateFormat.format(movieHistories.get(i).watchingDate));
    Picasso.with(context).load(Constants.BASE_URL + "/uploads/" + movieHistories.get(i).videoImgUrl).fit().centerInside().into(holder.videoImg);
  }
